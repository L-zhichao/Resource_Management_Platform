      ...
      <div style="padding: 5px 20px">
        ...
        <el-form :inline="true" :model="listQuery" class="demo-form-inline">
          ...
          >
            ...
            <el-radio-button label="asset">根据资产名称查询</el-radio-button>
          </el-radio-group>
        ...
        >
          ...
          <el-button
            ...
            @click="batchDelete"
            v-if="userAdministratorPermissions &&selection.length &gt; 0"
          >
            批量删除
          </el-button>
        </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      userAdministratorPermissions:
        util.cookies.get("userStatus") === "0" || false,
    };
  },
  created() {
    this.axiosdata();
  ...
  methods: {
    ...
    axiosdata() {
      ...
      axios
        .get("http://localhost:9090/fundsVo/page", {
          params: {
            ...this.listQuery,
            randomKey: this.randomKey, // 添加随机数参数
          },
          headers: {
    ...
    TJCollectionForm() {
      ...
      this.randomKey = Math.random(); // 更新随机数
    ...
    search() {
      ...
      if (this.searchType === "id") {
        url = `http://localhost:9090/fundsVo/fundsVo/${this.searchValue}`;
        axios
          .get(url)
          .then((response) => {
      ...
      } else if (this.searchType === "asset") {
        url = `http://localhost:9090/fundsVo/fundsVo/byName?name=${this.searchValue}`;
        axios
          .get(url)
          .then((response) => {
            this.result[0] = response.data.data;
    ...
    goBack() {
      ...
      this.searchValue = ""; // 清空查询内容
      this.searchType = "id"; // 恢复默认查询类型
      this.axiosdata(); // 刷新原始页面数据
    },
